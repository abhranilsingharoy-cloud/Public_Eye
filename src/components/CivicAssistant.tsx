...
* To enable real-time conversational processing, verify that your backend development server is running.`,
  ...
  const handleListenBriefing = async () => {
    ...
    try {
      ...
      setTtsMode('browser');
      const text = `Community Hero audio brief. We currently have ${issues.filter(i => i.status !== 'resolved').length} active reports in the Valencia-Dolores region. Potholes remain the highest category. Join the leaderboard to earn points and audit reports.`;
      setDigestText(text);
      window.speechSynthesis.cancel();
